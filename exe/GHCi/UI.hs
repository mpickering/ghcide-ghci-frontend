...
interactiveUI config srcs maybe_exprs = do
   ...
   let logger p = Logger $ \pri msg -> when (pri >= p) $ withLock lock $
                    T.putStrLn $ T.pack ("[" ++ upper (show pri) ++ "] ") S.<> msg
   sess <- GHC.getSession
   vfs <- liftIO $ makeVFSHandle
   hsc_env_eq <- liftIO $ newHscEnvEq sess []
   (ide_state, _) <- liftIO $  initialise def
                          mainRule
                          (error "idstate")
                          (const $ return ())
                          (logger minBound)
                          noopDebouncer
                          (defaultIdeOptions (return (const $ return ([], Just hsc_env_eq))))
                          vfs
   liftIO $ startGHCi (runGHCi srcs maybe_exprs)
...
loadModule' files = do
  ...
  exp_filenames <- mapM (fmap toNormalizedFilePath' . expandPath) filenames
--  let files' = zip exp_filenames phases
  res <- runSAction "LoadModule" (uses GetModIface exp_filenames)
