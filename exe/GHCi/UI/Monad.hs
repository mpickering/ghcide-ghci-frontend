...
reflectGHCi :: (IORef GHCiState) -> GHCi a -> IO a
reflectGHCi gs m = unGHCi m gs

reifyGHCi :: (IORef GHCiState -> IO a) -> GHCi a
reifyGHCi f = GHCi f
...
liftGhc m = do
  s <- getGHCiState
  (henv, res) <- GHCi $ \_ -> runGhcEnv (ghc_session s) m
  setGHCiState (s { ghc_session = henv })
  return res

runSAction :: (MonadIO m, HasGhciState m) => String -> Action a -> m a
runSAction herald action = do
  s <- ghcide_session <$> getGHCiState
  liftIO $ runAction herald s action
